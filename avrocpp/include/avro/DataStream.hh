...
#ifndef avro_DataStream_hh__
#define avro_DataStream_hh__
...
/**
 * Type-independent portion of DataStreamWriter.
...
public:
     ...
     */
    DataStreamWriterBase(std::auto_ptr<OutputStream> &stream, const ValidSchema& schema,
        size_t syncInterval);
...
class DataStreamWriter : boost::noncopyable {
    std::auto_ptr<DataStreamWriterBase> base_;
public:
    /**
     * Constructs a new data file.
     */
    DataStreamWriter(std::auto_ptr<OutputStream> &stream, const ValidSchema& schema,
        size_t syncInterval = 16 * 1024) :
        base_(new DataStreamWriterBase(stream, schema, syncInterval)) { }
...
public:
     ...
     * the DataStreamReaderBase object.
     */
    DataStreamReaderBase(std::auto_ptr<InputStream> &in);
...
class DataStreamReader : boost::noncopyable {
    std::auto_ptr<DataStreamReaderBase> base_;
public:
    /**
     * Constructs the reader for the given file and the reader is
     * expected to use the given schema.
     */
    DataStreamReader(std::auto_ptr<InputStream> &in, const ValidSchema& readerSchema) :
        base_(new DataStreamReaderBase(in)) {
     ...
     * expected to use the schema that is used with data.
     */
    DataStreamReader(std::auto_ptr<InputStream> &in) :
        base_(new DataStreamReaderBase(in)) {
        base_->init();
    }
     ...
     * from this reader.
     */
    DataStreamReader(std::auto_ptr<DataStreamReaderBase> base) : base_(base) {
        base_->init();
    }
     ...
     * from this reader.
     */
    DataStreamReader(std::auto_ptr<DataStreamReaderBase> base,
